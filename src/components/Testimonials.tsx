    ...
    <section className="section-padding bg-background">
      <div className="mx-auto max-w-7xl">
        ...
        >
          <CarouselContent className="-ml-4 touch-pan-y">
            {testimonials.map((testimonial, index) => (
              <CarouselItem key={index} className="pl-4 basis-[85%] md:basis-1/2 lg:basis-1/3">
                <Card className="bg-secondary/50 border border-border/50 h-full transition-all duration-300 hover:bg-secondary hover:shadow-lg hover:scale-[1.02]">
                  <CardContent className="p-6 sm:p-8 touch-manipulation">
          ...
          <div className="flex items-center justify-center gap-6 mt-8 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
            <CarouselPrevious className="relative static translate-y-0 h-9 w-9 transition-transform hover:scale-110" />
            <Button
              variant="outline"
              size="icon"
              onClick={() => setIsPaused(!isPaused)}
              className="h-10 w-10 sm:h-9 sm:w-9 transition-transform hover:scale-110"
            >
              {isPaused ? <Play className="h-5 w-5 sm:h-4 sm:w-4" /> : <Pause className="h-5 w-5 sm:h-4 sm:w-4" />}
            </Button>
            <CarouselNext className="relative static translate-y-0 h-9 w-9 transition-transform hover:scale-110" />
          </div>
        </Carousel>
        
        <div className="mt-16 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-8 items-center justify-center opacity-50">
          {[
            "/client-logos/logo1.svg",
            "/client-logos/logo2.svg",
            "/client-logos/logo3.svg",
            "/client-logos/logo4.svg",
            "/client-logos/logo5.svg",
            "/client-logos/logo6.svg"
          ].map((logo, index) => (
            <motion.img
              key={index}
              src={logo}
